...
public class FloatWindowService extends BaseFloatWindowService
{
    ...
    private class StickyUpdateTask extends AsyncTask
    {
...

        {
            if (isXcoordinate)
            {
                if (toValue == 0)
                {
                    for (; mLayoutParams.x > 0 && !mStickyHasReset; sleep())
                    {
                ...
                {
                    for (; mLayoutParams.x < toValue && !mStickyHasReset; sleep())
                    {
                        // log.e("x -0 =" + mLayoutParams.x);
                        mLayoutParams.x += STICKY_OFFSET;
                        publishProgress(mLayoutParams);
            ...
            else
            {
                if (toValue == 0)
                {
                    for (; mLayoutParams.y > 0 && !mStickyHasReset; sleep())
                    {
                ...
                {
                    for (; mLayoutParams.y < toValue && !mStickyHasReset; sleep())
                    {
                        // log.e("y -0 =" + mLayoutParams.y);
                        mLayoutParams.y += STICKY_OFFSET;
                        publishProgress(mLayoutParams);
    ...
    public void sendNotification(int type, int id, String s)
    {
        switch (type)
        {
            ...
            case SEND_NOTIFICATION:
                ...
                log.e("tttt s =" + mIntent);
                log.e("tttt=" + (mIntent.getIntExtra(AppUtils.KEY_SIZE, -555)));
                mIntent.putExtra(AppUtils.KEY_ISREOPEN, true);
                mIntent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
                PendingIntent pd = PendingIntent
                        .getService(this, 0, mIntent, 0);
    ...
    {
        // log.e("ooooo=" + (mIntent.getIntExtra(AppUtils.KEY_SIZE, -555)));
        // log.e("ooooo="
        // + (!mIntent.getExtras()
                        ...
                        .getBoolean(AppUtils.KEY_ISREOPEN, false))
        {
            ...
            // editor.putInt(AppUtils.KEY_SCREEN_W, mScreanW);
            // editor.putInt(AppUtils.KEY_SCREEN_H, mScreanH);
            // editor.commit();
